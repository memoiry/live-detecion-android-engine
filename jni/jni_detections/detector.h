...
private:
  ...
  inline void resizeRet(int img_width, int img_height)
  {
    for (auto i = 0; i < mRets.size(); i++)
    {
      auto &r = mRets[i];
      auto default_width_expand = r.width() * 0.3;
      if (r.left() > default_width_expand)
      {
        r.set_left(r.left() - default_width_expand);
      }
      else
      {
        r.set_left(0);
      }
      if (img_width > (r.right() + default_width_expand))
      {
        r.set_right(r.right() + default_width_expand);
      }
      else
      {
        r.set_right(img_width);
      }

      auto default_top_expand = r.height() * 0.5;
      auto default_bottom_expand = r.height() * 0.1;
      if (r.top() > default_top_expand)
      {
        r.set_top(r.top() - default_top_expand);
      }
      else
      {
        r.set_top(0);
      }
      if (img_height > (r.bottom() + default_bottom_expand))
      {
        r.set_bottom(r.bottom() + default_bottom_expand);
      }
      else
      {
        r.set_bottom(img_height);
      }
    }
  }
...
public:
  ...
  virtual inline int det(const cv::Mat &image)
  {
    if (image.empty())
      return 0;
    LOG(INFO) << "com_nanyun_dlib_PeopleDet go to det(mat)";
    if (image.channels() == 1)
    {
    ...
    resizeRet(img_width, img_height);
    return mRets.size();
  }
